import LOGO from './assets/images/Logo.svg'
import NAVIGATION from './assets/images/Navigation.svg'
import GET_STARTED from './assets/images/GetStarted.svg'
import ARROW from './assets/images/Arrow.svg'
import PROFIT from './assets/images/Profit.svg'
import LOSS from './assets/images/Loss.svg'
import BAR from './assets/images/Bar.svg'
import BAR2 from './assets/images/Bar2.svg'
import INFO from './assets/images/Info.svg'
import IMAGE1 from './assets/images/Image1.svg'
import IMAGE2 from './assets/images/Image2.svg'
import CARD2 from './assets/images/Card2.svg'
import NEXT_ARROW from './assets/images/NextArrow.svg'
import BUY_BAR from './assets/images/BuyBar.svg'
import PROFIT_BAR from './assets/images/ProfitBar2.svg'
import HOLD_BAR from './assets/images/HoldBar.svg'
import SELL_BAR from './assets/images/SellBar.svg'
import CARD_IMAGE1 from './assets/images/CardImage1.svg'
import CARD_IMAGE2 from './assets/images/CardImage2.svg'
import PIE_CHART from './assets/images/PieChart.svg'
import PERSON1 from './assets/images/Person1.svg'
import PERSON2 from './assets/images/Person2.svg'
import PERSON3 from './assets/images/Person3.svg'
import MENU from './assets/images/Menu.svg'
import LOSS_BAR from './assets/images/MidBar.svg'
import HOLD_BAR2 from './assets/images/LossBar2.svg'
import PL1 from './assets/images/PL1.svg'


const NAVBAR_OPTIONS = [
    {optionName:'Crypto Taxes',link:'/crypto-taxes'},
    {optionName:'Free Tools',link:'/free-tools'},
    {optionName:'Resource Center',link:'/resource-center'}
]

const ROW1 = [
    {title:"Bitcoin Price",desc:"$16,815.46"},
    {title:"24h Low / 24h High",desc:"$16,382.07 / $16,874.12"},
    {title:"7d Low / 7d High",desc:"$16,382.07 / $16,874.12"},
    {title:"Trading Volumn",desc:"$23,24,202,782"},
    {title:"Market Cap Rank",desc:"#1"}
]
const ROW2 = [
    {title:"Market Cap",desc:"$323,507,290,047"},
    {title:"Market Cap Dominance",desc:"38.343%"},
    {title:"Volumn / Market Cap",desc:"0.0718"},

]

const BOX_OPTIONS = [
    {
        image:IMAGE1,
        title:'Lorem ipsum dolor sit amet consectetur. Dui vel quis dignissim mattis enim tincidunt.',
        desc:'Lorem ipsum dolor sit amet consectetur. Ac phasellus risus est faucibus metus quis. Amet sapien quam viverra adipiscing condimentum. Ac consectetur et pretium in a bibendum in. Sed vitae sit nisi viverra natoque lacinia libero enim.'
    },
    {
        image:IMAGE2,
        title:'Lorem ipsum dolor sit amet consectetur. Dui vel quis dignissim mattis enim tincidunt.',
        desc:'Lorem ipsum dolor sit amet consectetur. Ac phasellus risus est faucibus metus quis. Amet sapien quam viverra adipiscing condimentum. Ac consectetur et pretium in a bibendum in. Sed vitae sit nisi viverra natoque lacinia libero enim.'
    },
    {
        image:IMAGE1,
        title:'Lorem ipsum dolor sit amet consectetur. Dui vel quis dignissim mattis enim tincidunt.',
        desc:'Lorem ipsum dolor sit amet consectetur. Ac phasellus risus est faucibus metus quis. Amet sapien quam viverra adipiscing condimentum. Ac consectetur et pretium in a bibendum in. Sed vitae sit nisi viverra natoque lacinia libero enim.'
    },
]

const TEAM = [
    {image:PERSON1,name:"Jone Smith",dest:"Designation here",message:'Lorem ipsum dolor sit amet consectetur. In justo rutrum sit sit fermentum ut libero hendrerit id. Tellus sit ornare netus sagittis in nunc convallis mattis maecenas. Tempus arcu leo sociis laoreet nec neque sed pellentesque viverra. Consectetur proin amet ut id facilisi quis consectetur. Tellus gravida ultricies feugiat sed eu egestas dolor est ipsum. Malesuada etiam mi gravida praesent interdu'},
    {image:PERSON2,name:"Jone Smith",dest:"Designation here",message:'Lorem ipsum dolor sit amet consectetur. In justo rutrum sit sit fermentum ut libero hendrerit id. Tellus sit ornare netus sagittis in nunc convallis mattis maecenas. Tempus arcu leo sociis laoreet nec neque sed pellentesque viverra. Consectetur proin amet ut id facilisi quis consectetur. Tellus gravida ultricies feugiat sed eu egestas dolor est ipsum. Malesuada etiam mi gravida praesent interdu'},
    {image:PERSON3,name:"John Smith",dest:"Designation here",message:'Lorem ipsum dolor sit amet consectetur. In justo rutrum sit sit fermentum ut libero hendrerit id. Tellus sit ornare netus sagittis in nunc convallis mattis maecenas. Tempus arcu leo sociis laoreet nec neque sed pellentesque viverra. Consectetur proin amet ut id facilisi quis consectetur. Tellus gravida ultricies feugiat sed eu egestas dolor est ipsum. Malesuada etiam mi gravida praesent interdu'},
]


const TAB_OPTIONS = ["Overview","Fundamentals","News Insights","Sentiment","Team","Technical","Tokenomics"]




export {NAVBAR_OPTIONS,LOGO,NAVIGATION,GET_STARTED,ARROW,PROFIT,LOSS,TAB_OPTIONS,BAR,BAR2,INFO,ROW1,ROW2,BOX_OPTIONS,IMAGE1,CARD2,NEXT_ARROW,BUY_BAR
,HOLD_BAR,SELL_BAR,CARD_IMAGE1,CARD_IMAGE2,PIE_CHART,TEAM,MENU,PROFIT_BAR,HOLD_BAR2,LOSS_BAR,PL1}